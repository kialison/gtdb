...
description: "Learn how prices and order books represent trading activity on event markets, including order books, best prices, midpoints, spreads, last trades, and price history."
---

Each event market outcome is represented by a token traded on the CLOB. Its price reflects what traders are willing to pay for that outcome, while its order book shows the resting bids and asks.

The examples below assume you already have a market object from which to read the outcome token IDs. To find or fetch one, see [Discover Markets](/event-markets/untitled-page-11).
